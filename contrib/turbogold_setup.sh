#!/bin/bash
# This script will install all dependencies to run a Turbogold (TBG) Masternode.
...
read vpsip
touch /root/.Turbogold/Turbogold.conf
echo -e "rpcuser=$usrnam \nrpcpassword=$usrpas \nrpcallowip=127.0.0.1 \nlisten=1 \nserver=1 \ndaemon=1 \nmasternode=1 \nlogtimestamps=1 \nmasternodeprivkey=$mngenkey \nexternalip=$vpsip:15802 \n" > /root/.Turbogold/Turbogold.conf
echo '*** Done 4/5 ***'
sleep 2
echo '*** Step 5/5 ***'
