...
int nAnonymizeTurbogoldAmount = 1000;
...
#else
    const char* pszModule = "Turbogold";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Turbogold
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Turbogold
    // Mac: ~/Library/Application Support/Turbogold
    // Unix: ~/.Turbogold
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Turbogold";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Turbogold.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Turbogoldd.pid"));
