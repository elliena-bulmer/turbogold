...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTurbogoldAmount"))
        settings.setValue("nAnonymizeTurbogoldAmount", 1000);
    nAnonymizeTurbogoldAmount = settings.value("nAnonymizeTurbogoldAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeTurbogoldAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeTurbogoldAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeTurbogoldAmount:
            return QVariant(nAnonymizeTurbogoldAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeTurbogoldAmount:
            nAnonymizeTurbogoldAmount = value.toInt();
            settings.setValue("nAnonymizeTurbogoldAmount", nAnonymizeTurbogoldAmount);
            emit AnonymizeTurbogoldAmountChanged(nAnonymizeTurbogoldAmount);
            break;
        default:
            break;
        }
