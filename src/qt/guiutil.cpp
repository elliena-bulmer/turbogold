...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Turbogold"))
        return false;
...
{
    // Convert Turbogold:// to Turbogold:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Turbogold://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Turbogold:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Turbogold.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Turbogold.lnk
...
{
    return GetAutostartDir() / "Turbogold.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Turbogold.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Turbogold\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Turbogold app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Turbogold app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Turbogold-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Turbogold-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Turbogold-Qt"));
