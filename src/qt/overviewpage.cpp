...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeTurbogoldAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTurbogoldAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeTurbogoldAmount = strAnonymizeTurbogoldAmount.remove(strAnonymizeTurbogoldAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTurbogoldAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeTurbogoldAmount*COIN) nMaxToAnonymize = nAnonymizeTurbogoldAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeTurbogoldAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeTurbogoldAmount));
        strAnonymizeTurbogoldAmount = strAnonymizeTurbogoldAmount.remove(strAnonymizeTurbogoldAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTurbogoldAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeTurbogoldAmount == 0){
