...
//
// Bootup the masternode, look for a 800 TURBOGOLD input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Turbogold.conf */
...
{
    CTurbogoldAddress address(collateralAddress);
