...
void Shutdown()
{
    ...
    RenameThread("Turbogold-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Turbogold.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Turbogoldd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeTurbogoldamount=<n> " + _("Keep N Turbogold anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Turbogold is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Turbogold is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Turbogold version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Turbogold server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Turbogold") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Turbogold to complete") << "\n";
    ...
    nAnonymizeTurbogoldAmount = GetArg("-anonymizeTurbogoldamount", 0);
    if(nAnonymizeTurbogoldAmount > 999999) nAnonymizeTurbogoldAmount = 999999;
    if(nAnonymizeTurbogoldAmount < 2) nAnonymizeTurbogoldAmount = 2;
    ...
    LogPrintf("Anonymize Turbogold Amount %d\n", nAnonymizeTurbogoldAmount);
